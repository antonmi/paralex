...
module JobReactor
  module Distributor
    ...
    def get_connection(hash)
      if hash['node']
        node_connection = connections.find { |con| con.name == hash['node'] && con.name != hash['not_node'] }
        if node_connection && node_connection.available?
          node_connection
        else
          JR.config[:always_use_specified_node] ? nil : connections.find { |con| con.available? && con.name != hash['not_node'] }
        end
      else
        connections.find { |con| con.available? && con.name != hash['not_node'] }
      end
    end
  end
end
