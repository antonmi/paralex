...
module JobReactor
  module RedisMonitor
    ATTRS = %w(id name args last_error run_at failed_at attempt period make_after status distributor on_success on_error defer).freeze
    ...
    def jobs_for(name, to_be_retried = false)
      ...
      keys.each do |key|
        hash = load(key)
        if to_be_retried
          result[key] = hash if hash['status'] != 'complete' && hash['status'] != 'cancelled' && hash['status'] != 'failed'
        else
          result[key] = hash
        end
      end
    ...
    def load(key)
      ...
      %w(attempt period make_after).each do |attr|
