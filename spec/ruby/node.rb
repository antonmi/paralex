...
module JobReactor
  class Node
    ...
    def start
      ...
      EM.start_server(*config[:server], Server, self, storage)
      config[:distributors].each do |distributor|
      ...
      run_job = proc do
        if hash['make_after'].to_i > 0
          EM::Timer.new(hash['make_after']) do
            storage.load(hash) { |hash| do_job(JR.make(hash)) }
          end
        else
          EM.next_tick do
            storage.load(hash) { |hash| do_job(JR.make(hash)) }
    ...
    def rescue_job(e, job)
      ...
      storage.save(job) do |job|
        begin
          ...
          job['args'][:errback_error] = e # So in args you now have :error and :errback_error
          complete_rescue(job)
        end
      end
    end
    ...
    def retry_jobs
      storage.jobs_for(name) do |job_to_retry|
        job_to_retry['args'][:retrying] = true
    ...
    def report_success(job)
      ...
      port = port.to_i
      distributor = connections[[host, port]]
      data = { success: { callback_id: job['on_success'], args: job['args'] } }
      data[:success][:do_not_delete] = true if job['period'] && job['period'].to_i > 0
