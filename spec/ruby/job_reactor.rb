...
module JobReactor
  ...
  class << self
    ...
    def enqueue(name, args = {}, opts = {}, success_proc = nil, error_proc = nil)
      ...
      hash['period'] = opts[:period] if opts[:period]
      opts[:after] = (opts[:run_at] - Time.now) if opts[:run_at]
      hash['make_after'] = (opts[:after] || 0)

      hash['node'] = opts[:node] if opts[:node]
      hash['not_node'] = opts[:not_node] if opts[:not_node]

      hash['distributor'] = JR::Distributor.server

      hash['defer'] = opts[:defer] ? 'true' : 'false'
    ...
    def make(hash) # new job is a Hash
      fail NoSuchJob unless jr_job = JR.jobs[hash['name']]
    ...
    def add_succ_feedbacks!(hash, callback)
      ...
      succ_feedbacks[feedback_id] = callback
    ...
    def add_err_feedbacks!(hash, errback)
      distributor = JR::Distributor.server
      feedback_id = "#{distributor}_#{Time.now.utc.to_f}"
      err_feedbacks[feedback_id] = errback
